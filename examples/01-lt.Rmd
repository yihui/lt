...
Instead of fixing the number of decimal places, use `sig_digits` to keep a
fixed number of significant digits. This is handy when a column spans several
orders of magnitude, since small and large values each retain the same number
of meaningful digits:

```{r}
d3 = data.frame(
  Particle = c("Electron", "Proton", "Alpha"),
  Mass = c(0.00054858, 1.00727647, 4.00150618)
)
lt(d3) |> lt_format(~ Mass, sig_digits = 3)
```

`sig_digits` cannot be combined with `decimals`.
...
A footnote can be attached to any of six locations via the `where`
argument: the `title`, the `subtitle`, a `column` label, a `spanner`
label, a row `group` label, or one or more `body` cells. The example
below shows all six at once:
...
lt(d) |>
  ...
  # specific body cells (column + rows)
